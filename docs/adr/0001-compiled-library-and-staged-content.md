...
Sandcastle-drain ships as an npm-installable library (`npx sandcastle-drain <subcommand>`). The implementer and reviewer agents it spawns run inside per-issue Sandcastle worktrees and need to read three things at runtime:
...
Compile to `dist/` and stage library content into the host's `.sandcastle-drain/staged/` at drain time. Render prompts in memory and pass them as inline strings.
...
2. **Runtime staging.** `src/stage.ts` exposes `stage(cwd)`, which the CLI calls once before the drain loop. It copies the library's `content/principles/` and `content/agent-docs/` into `<host-cwd>/.sandcastle-drain/staged/{principles,agent-docs}/`. Each call wipes the prior `.sandcastle-drain/staged/` tree so a library upgrade is reflected immediately.
3. **Prompt rendering.** `src/render-prompt.ts` reads the bundled `dist/prompts/{implementer,reviewer}.md.tpl`, substitutes simple `{{KEY}}` placeholders, and returns the rendered string. The orchestrator passes the result as `prompt: <string>` to `sandcastle.run()`. The host's `.sandcastle-drain/` directory never sees a `prompt.md` / `reviewer.md` file.
4. **Plumbing.** Each `run()` invocation bind-mounts `<host-cwd>/.sandcastle-drain/staged/` into the sandbox at the same relative path, read-only, via sandcastle's docker `mounts` option. The agent sees `.sandcastle-drain/staged/...` from inside the worktree exactly as if it had been copied in.
5. **Prompt path references.** Path references inside the prompt templates (e.g. principle filenames the reviewer eager-loads) point at `.sandcastle-drain/staged/principles/...` — what the agent sees from inside the worktree — not the library-source `src/content/principles/...`.
...
- **`.sandcastle-drain/staged/` is a runtime artifact and is gitignored.** A host project should never commit it — it gets rewritten every drain. No prompt files materialize on the host filesystem at all.
- **Prompt path references are decoupled from this repo's directory layout.** Renaming `src/content/principles/` here would not break agents in host projects; only the prompt body's `.sandcastle-drain/staged/principles/...` references need to stay stable.
