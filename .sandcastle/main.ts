...
// derive it from the repo root's directory name so the wrapper stays portable
// — drop this template into any project and it picks up the right image.
const IMAGE_NAME = `sandcastle:${basename(REPO_ROOT)}`;
...
async function probeLabels(): Promise<string | null> {
  let existing: ReadonlySet<string>;
  try {
    const stdout = await gh(['label', 'list', '--limit', '200', '--json', 'name']);
    const rows = JSON.parse(stdout) as ReadonlyArray<{ name: string }>;
  ...
  for (const def of missing) {
    try {
      ...
      return `failed to create label '${def.name}': ${(err as Error).message}`;
    }
  }
  return null;
}
