...
On Windows, `sandcastle.run()` throws `error: failed to delete '.sandcastle/worktrees/agent-issue-N': Function not implemented` _after_ the agent has committed. (Path is `.sandcastle/`, not `.sandcastle-drain/` — sandcastle owns this directory; the wrapper's own dir is `.sandcastle-drain/`.) This is the **expected** exit path for any drain that runs `pnpm install`, not a failure mode. The wrapper:
