...
Nine labels that exist only for the sandcastle-drain wrapper at `src/orchestrator/main.ts`. `sandcastle`, `blocked`, `retry`, and `ui` are user-applied; `in-progress`, `needs-review`, `priority`, `oversized`, and `skipped-this-run` are wrapper-managed — don't touch them by hand unless you're recovering from a crashed run.
...
| `ui`           | You apply manually to opt this issue's UI work into the Visual-Iteration Engine. The engine runs only when the issue carries `ui` **and** the project ships a visual rubric + preview-adapter config; it iterates `capture → critique → edit → recapture` before the reviewer. See ADR 0004.                                                                          | Manually — not wrapper-managed.                                                                                     |
...
3. Applies labels:
   ...
   | Commits + CI green + reviewer `PASS` + visual ceiling-fail (`ui` issue) | remove `in-progress` + `sandcastle`, add `needs-review` (branch preserved, dependents skipped). The visual verdict blocks auto-merge, but a visual fail ≠ rejection — the editor's commits are the work, so the branch is kept, not discarded. See ADR 0004. |
